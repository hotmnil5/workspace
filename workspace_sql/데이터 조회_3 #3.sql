...
-- 데이터의 변화가 생기는 경우 해달 변화가 잘못이 없는지 한 번 확인할 기회를 줌
-- 변화된 데이터가 잘못되지 않았으면 COMMIT; 명령어로 데이터 변화를 확정지을 수 있음
...
INSERT INTO test_member (MEM_NUM, MEM_NAME, MEM_ADDR, MEM_AGE) VALUES (11, 'KIM', '울산', 30);
...
-- 아래 쿼리는 MEM_NUM 에 NULL이 들어가는데
-- MEM_NUM은 기본키(PRIMARY KEY)라 NULL값이 들어갈 수 없어 오류발생
