...
public class Bank {
    ...
    public Client findClient(String accNumber, String name) {
        for (Client client : clientList) {
            if (client.getAccInfo().getAccNUmber().equals(accNumber) && client.getName().equals(name)) {
                return client;
            }
        }
    ...
    //메서드를 정의하시오.
    public List<Client> findBalance(int balance) {
        List<Client> result = new ArrayList<>();
        for (Client client : clientList) {
            if (client.getAccInfo().getBalance() > balance) {
                result.add(client);
            }
        }
        return result;
