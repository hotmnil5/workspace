...
const RegItem = () => {
  ...
  function insertItem(){
    ...
    const fileConfig = {headers : {'Content-Type' : 'multpart/form-data'}}

    //위의 설정코드를 axios 총신할 때 추가하면
    ...
    <div className='reg-item-div'>
      ...
      <div className='input-element'>
        ...
        <div><input type='text' className='form-control'
              name='itemName' onChange={(e)=>{changeInsertItemData(e)}}/></div>
      </div>
      <div className='input-element'>
        <div>상품 가격</div>
        <div><input type='text' className='form-control' 
              name='itemPrice' onChange={(e)=>{changeInsertItemData(e)}}/></div>
      </div>
      <div className='input-element'>
        <div>상품 소개</div>
        <div>
          <textarea className='form-control' rows={7} 
          name='itemIntro' onChange={(e)=>{changeInsertItemData(e)}}>
          </textarea>
        </div>
      </div>
      <div className='input-element'>
